...
export class UsersService {
  ...
  async alterarPerfil(id: string, dto: AlterarPerfilDto, req: Request): Promise<void> {
    ...
    try {
      ...
      throw new HttpException(e?.response?.data?.message ?? 'Erro ao alterar perfil.', e?.response?.status ?? 500);
    }
  }
  ...
  async desativar(id: string, req: Request): Promise<void> {
    ...
    try {
      ...
      const e = err as AxiosErr;
      throw new HttpException(e?.response?.data?.message ?? 'Erro ao desativar usuário.', e?.response?.status ?? 500);
    }
  }
  ...
  private async get(path: string, req: Request): Promise<unknown> {
    ...
    try {
      ...
      const e = err as AxiosErr;
      throw new HttpException(e?.response?.data?.message ?? 'Erro ao consultar usuário.', e?.response?.status ?? 500);
    }
  }
  ...
  private async post(path: string, body: unknown, headers: Record<string, string>): Promise<unknown> {
    try {
      ...
      const e = err as AxiosErr;
      throw new HttpException(e?.response?.data?.message ?? 'Erro ao criar usuário.', e?.response?.status ?? 500);
    }
  }
}
